...
export async function GET(request: Request, context: RouteContext) {
  ...
  try {
    ...
    const authUser = await verifyAuth(request);
    const noStoreJson = <T>(payload: T, init?: ResponseInit): Response =>
      withNoStore(Response.json(payload, init));
    ...
    if (error instanceof AuthError) {
      if (error.statusCode === 403) {
        logAuthorizationFailure({
          method: "GET",
          endpoint:
            requestErrorContext.endpoint ?? `GET ${requestUrl.pathname}`,
    ...
    if (isFirestoreFailedPrecondition(error)) {
      console.error("Firestore failed precondition on slots query", {
        endpoint: requestErrorContext.endpoint ?? requestUrl.pathname,
        organizationId: requestErrorContext.organizationId,
...
export async function PATCH(request: Request, context: RouteContext) {
  ...
  const patchErrorContext = {
    endpoint: `PATCH ${requestUrl.pathname}`,
    organizationId: "unknown",
  };
...
export async function DELETE(request: Request, context: RouteContext) {
  ...
  const deleteErrorContext = {
    endpoint: `DELETE ${requestUrl.pathname}`,
    organizationId: "unknown",
  };
