...
async function sameTime(argv: string[]) {
  ...
  const topic = `あなたのみらい予報 ブレイクアウトテスト (same-time) ${date}`;
...
async function differentTime(argv: string[]) {
  ...
  const topic1 = `あなたのみらい予報 ブレイクアウトテスト (different-time Room1 ${h1}:00) ${date}`;
  ...
  const topic2 = `あなたのみらい予報 ブレイクアウトテスト (different-time Room2 ${h2}:00) ${date}`;
