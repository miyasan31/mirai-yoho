...
`requires` に書いた動的パラメタが解決できない画面は、警告を出してスキップされます。次の画面はデータ依存が強いので、撮影前にシードの投入状況を確認してください。

| 画面 | 必要なデータ |
|---|---|
| 占い師コンソール「鑑定書」「鑑定書の発行」 | 鑑定の終了時刻を過ぎた予約が 1 件以上 |
| 運営コンソール「占い師詳細」「占い師編集」 | 占い師が 1 名以上 |
| 予約サイト「鑑定の評価」 | 鑑定終了後・評価期間内で未評価の予約が 1 件以上 |
| 予約サイト「鑑定書」 | 発行済みの鑑定書が 1 件以上 |

local ならいずれも `make seed-local ADMIN=you@example.com` のシードに含まれています。
