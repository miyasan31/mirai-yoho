...
- アーカイブと公開中の施行日はどちらも過去日です。未来日にすると `findLatestPublished` が空になり、予約フローの規約同意チェックが通りません。下書きは施行日を持たず、console で公開するときに決めます（新規組織を作る `create-organization.ts` も下書きで投入します）
