...
const PUBLIC_CACHE_CONTROL_BY_KEY: Partial<Record<PublicCacheKey, string>> = {
...
export function getPublicCacheControl(
  ...
  return PUBLIC_CACHE_CONTROL_BY_KEY[cacheKey] ?? undefined;
}
...
export function withPublicCacheControl(
  ...
  const cacheControl = getPublicCacheControl(cacheKey);
  if (cacheControl) {
