# Arc - みらい予報 — 製品要件定義書（PRD）

> Version 0.5 | 2026-03-22 | Draft（未決事項 0 件）

> **注記（2026-07-28 追記）**: 本書が前提とする以下の要件は、その後の実装で **廃止・変更済み**。現行仕様は `doc/DDD_DESIGN.md` を参照すること。
>
> | 本書の記述 | 現行 |
> |---|---|
> | サービス名「Arc - みらい予報」 | **「あなたのみらい予報」**（運営: 一般社団法人JKK）。オンライン**鑑定**サービス |
> | 「相談員」 | **「占い師」**（コード上の識別子は `Consultant` のまま） |
...
| 0.5 | 2026-03-22 | システム名を Arc - みらい予報 に変更。被相談者を顧客に統一。相談員ログイン追加。管理者 2 ロール制（super_admin / operator）導入。本決済を深夜バッチ自動化。相談員プロフィール公開・顧客による相談員選択を追加。Firebase Storage・Cloud Scheduler を技術スタックに追加。全 22 画面に拡張 |