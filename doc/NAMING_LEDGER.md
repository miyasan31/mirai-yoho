...
> **呼称について（2026-07-31）**: 予約直後に確保する 15 分枠の日本語呼称を「バッファ」→「**準備時間**」に変更した。運営者・占い師に「バッファ」が伝わらないため、UI・マニュアル・ドキュメントの表記を統一する。英語・コード上の識別子（`bufferSlotIds` / `getBookingBufferMinutes` / `BUFFER_SLOT_COUNT` など）は**変更しない**方針のため、本台帳の識別子は現行のままで正しい。
