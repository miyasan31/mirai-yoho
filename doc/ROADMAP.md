...
> **注記（2026-08-07）**: Phase 6 を PR #228 時点まで反映済み。
...
| `[x]` | Playwright による操作マニュアル PDF 生成ツール（consultant / console / user）。サービス間の関連性を追記し全機能を網羅 | PR #169, #172, #177, #178, #202 |
| `[x]` | マイページで電話番号を保存し予約フォームに初期表示 | PR #173 |
| `[x]` | 組織の切り替えを URL の組織 ID 基準に統一 | PR #175 |
| `[x]` | ブレイクアウトルームを予約単位に分割し、割当状況の確認画面を追加（確認画面は後に廃止） | PR #179, #180 |
...
| `[x]` | マイページのプロフィールから退会画面への導線を追加 | PR #203 |
| `[x]` | ドキュメント 10 件を実装に合わせて最新化 | PR #204（#186〜#195） |
| `[x]` | 命名台帳の未実施合意と過剰改名を是正 | PR #205（#196〜#198） |
| `[x]` | 不具合修正（今後の予約を開始日の昇順に整列、204 No Content で JSON パースエラーになる問題） | PR #207, #208 |
| `[x]` | マイページの Zoom 連携ページを外部連携ページに再構成 | PR #209 |
| `[x]` | マイページのプロフィールでメールアドレスと電話番号を必須化 | PR #210 |
| `[x]` | マイページの「予約に戻る」を予約可能な店舗一覧ページに置き換え（`apps/user/src/config/bookable-organizations.ts`） | PR #211 |
| `[x]` | console のブレイクアウトルーム画面を廃止し、ポリシー画面の表示名を「文書管理」に変更 | PR #212 |
| `[x]` | 全アプリのログアウトボタンを赤色表示に統一 | PR #213 |
...
| `[x]` | 15分の準備時間の日本語呼称を「バッファ」から「準備時間」に統一（コード上の識別子は変更せず） | PR #227 |
| `[x]` | 操作マニュアル生成 config を鑑定書・評価・精算書の追加に追従（データ依存の強い画面を README に明記） | PR #228 |
