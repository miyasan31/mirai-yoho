...
3. **マイページ** … プロフィール / 予約一覧 / 鑑定の評価 / 鑑定書 / 外部連携 / クーポン
4. **規約・退会** … 規約類と退会
...
- **予約 ID など動的 param が取れない**: 対象 SPA にサンプルデータ（予約 1 件以上）が入っているか確認、または `MANUAL_ORG_ID` / config の `defaultOrgId` を設定。鑑定書・評価の画面は終了済み予約や発行済み鑑定書がないと解決できない（`scripts/manual/README.md` の「データがないと撮れない画面」を参照）
